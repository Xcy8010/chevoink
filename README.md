...
5. **一键导出**：沉浸模式工具栏、「…」更多菜单与手机端「更多」均可发起，勾选导出范围（规划/目录/章节/作品信息以及发布建议，章节可逐章自选），服务端打包 zip 直接下载，附 AI 按番茄小说官方词表生成的发布建议；也可在对话里让 Agent 按需导出（只导出指定章节、排除某部分内容）；
6. 章节写完点击发布，读者端即刻可见；支持定时追更与章节管理。

## ✨ 功能一览

- **阅读区**：书城首页（轮播、榜单、分类推荐）、书架与阅读进度云同步、沉浸式阅读器、TTS 听书
- **创作区**：小说/章节管理、AI 写作 Agent（流式事件、工具调用、知识集 Skill、默认最大权限、图片/文件附件、视觉看图、pdf/docx/txt/md 参考资料读取、联网搜索与网页阅读、站内作品参考（查看全站已上架作品与本人未公开作品、按标签/题材识别类似作品、站内无果联网补充，支持二创/写序章）、跨会话记忆）、AI 封面生成（远程直链自动落盘）、一键导出 zip（规划/目录/章节/作品信息以及发布建议，附番茄小说官方词表发布建议，Agent 工具可按需裁剪）
