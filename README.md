...
- **创作区**：小说/章节管理、AI 写作 Agent（结论与正文均实时流式输出、写入期编辑锁、Work 追踪、Harness 式紧凑工具过程、作品技能、图片/文件附件、联网与站内调研、跨会话记忆）、基于 low-reasoning AI 的全作品关系网、AI 封面生成、一键导出 zip
- **社区**：帖子与话题系统、推荐算法、评论/点赞/收藏、关注与粉丝、私信与在线状态
- **账号体系**：手机号验证码登录（腾讯云短信）、HttpOnly Cookie 会话 + Bearer 备选通道（安卓壳杀后台不掉登录）
- **管理后台**：数据看板、用户/作品/内容治理、Token 用量排行与创作任务下钻、联网/生图调用统计、移动端适配
