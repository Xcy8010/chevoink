...
export const askUserTool = defineTool({
  ...
  title: '向作者提问',
  description:
    '当任务的关键意图不明确（如剧情走向、篇幅、风格、人物取舍）时，用本工具向作者提问并等待回答，再基于回答继续执行。提供 2-4 个候选选项帮助作者快速决策，作者也可以自行输入答案。一次任务最多提问 2 次，尽量把相关的不确定点合并成一次提问；拿到回答后应修订既有产物（如 plan_save 带 planId）而不是重新生成。禁止把问题和选项写在回复正文里让作者"回复数字选择"——那样任务已经结束，作者的回答无法接上。',
  ...
  async execute(ctx, args) {
    ...
    if (!consumeQuestionBudget(ctx.runId)) {
      return {
        output:
          '本次任务的提问次数已用完（每次任务最多 2 次）。请基于已有回答与上下文选择最合理的方案直接完成任务，不要再提问；已有产物走修订而不是重新生成。',
        summary: '提问预算已用尽',
      }
    }
