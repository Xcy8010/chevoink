...
2. 精准修改：优先 chapter_edit_range 传 oldText（逐字拷贝要替换的原文，系统自动定位，禁止自己数字数算下标）只改目标区间，禁止为局部修改全文重写；只有作者明确要求全章重写时才用 chapter_write。
3. 收尾：一句话说明改了哪里、怎么改的。
...
3. 改写：chapter_edit_range 传 oldText 锚点逐段就地修改（系统自动定位，禁止数字数算下标），删除优先于替换，空洞情绪换成具体动作与物件，长短句交替，保留原剧情、视角与人物声口。
...
const platformReferenceSkill: AgentSkill = {
  ...
  trigger: {
    modes: ['plan', 'build'],
    pattern: /(查看|参考|借鉴|学习|二创).{0,12}(作品|书|《)|(二创|同人).{0,8}(修改|写|序章)|(搜索|找|推荐).{0,8}(类似|相似|同类|同类型)/,
  },
...
3. 类似作品诉求：从参考作品标签/分类/简介提炼特征词，用特征词搜同类并对比简介判相似（严禁拆书名逐字穷举）；站内无果用 web_search 搜站外推荐。
4. 参考执行：提炼风格、结构、开篇钩子等可参考要素，再用既有工具执行当前写作任务；二创/改写成果用 chapter 工具写入当前作品，正文禁止贴在回复里。
5. 收尾：不超过 2 句话汇报参考了哪部作品与完成情况。
铁律：参考他人作品仅限已上架内容；二创/写序章必须先读参考作品的简介与相关章节再动笔，禁止盲写。
自查清单：是否先读参考作品再行动？收尾是否注明来源？`,
}
