...
> 实施状态（2026-08-25）：已完成并部署生产。Volume、ChangeSet、Memory2、Skill2、DualWorkspace 五项开关可独立关闭，并支持按用户 ID 灰度；前端入口、服务端接口和 Agent 工具同时受控，关闭不删除数据。新增默认 dry-run 的结构修复、并发索引重建、记忆重提取和 7 日灰度指标工具，以及完整部署/SLO/分级回滚/数据导出/线上验收手册。四闸、生产依赖审计、5 项生产迁移、PM2/nginx 重载、API 健康检查与公网 200 均通过；旧客户端继续使用全书 orderIndex 只读，原稿导出链路保持不变。
