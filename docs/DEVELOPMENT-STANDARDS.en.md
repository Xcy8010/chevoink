...
History receipts must not be embedded in assistant prose. Supply separate read-only data with real status and object IDs; remove legacy assistant receipt echoes without changing user text. Protocol correction requests native `tool_choice=required` without expanding permissions or retry budgets; tool-free wrap-up omits it. Explicit next-chapter delivery requires nonempty persisted content and a committed bridge matching the current revision in the same task contract. Test persisted resume, stale revisions and cross-task rejection, not just model claims or todo state.
