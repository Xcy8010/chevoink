...
# 2026-09-10 desktop candidate

Windows 1.0.4 moves native menu actions into Windows-only Settings using origin/label/ACL checks and a fixed action whitelist. Existing-task cold deep links no longer manufacture blank task windows or fall back to another task; failed resolution keeps the original target for retry and does not overwrite the workspace snapshot. A branded packaged loading page shares the native WebView background. These are candidate changes, not completed deployment or physical-device acceptance. Android 1.0.7's existing APK has the original signer, package identity, code10 and matching UA, but the production update manifest was still at 1.0.6 and needs a separate static-channel publication.
