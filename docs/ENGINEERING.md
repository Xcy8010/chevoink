...
- **模型窗口感知压缩（P0）**：每次主 Agent 与内嵌子 Agent 请求发送前，以混合中英文保守 token 估算统计 system/user/assistant/tool、原生 reasoning、`tool_calls.arguments`、图片占位和完整工具 schema；65% 提醒、72% 开始压缩，并为输出与估算误差保留安全区。第一阶段保留调用协议与 callId，只压缩久远工具参数/输出；若仍超过硬阈值，第二阶段把完整旧工具轮替换为确定性收据，最近 8 条工具结果保持原样。仍无法落入安全窗口时不向供应商发送必失败请求，而是落库安全收尾并保留继续执行入口。实现：`api/lib/agent/context-budget.ts`。
- **跨模型一致性**：DeepSeek、GLM 与用户自接 OpenAI-compatible 模型共用上述本地确定性压缩，不依赖某家供应商专属 API。`AiModelConfig.metadata.contextWindowTokens` 可由管理端或自定义模型设置填写（16K–4M），运行中预算、跨 Run 历史额度和上下文占用卡均采用当前模型窗口；未配置时回退 `AGENT_CONTEXT_WINDOW_TOKENS`。OpenAI Responses 的远端 `/responses/compact` 是可选的供应商能力，不直接混入当前 Chat Completions 工具协议。
- **持久化检查点正确性**：跨 Run 自动/手动压缩保留最近 24 条消息，并将目标、有效硬约束、决策、已完成项和工具凭据写入 `ContextCheckpoint`。触发比例只计算“上一检查点摘要 + 当前有效窗口”，不再被已压缩历史永久抬高；中文采用保守估算；边界以 `createdAt + messageId` 排序，避免同毫秒消息遗漏；同会话压缩在进程内单飞，并以数据库 `(sessionId, sourceHash)` 唯一键阻止多实例重复落库，手动端点还会在服务端拒绝运行中会话。无新内容时返回“无需压缩”，不会伪报新版本。多次压缩会在 UI 提醒早期措辞细节可能淡化。
...
当前共有 **75 个测试文件、424 个用例**。CI 提供 PostgreSQL 16，执行全部数据库集成组；无数据库的本地环境会自动跳过 DB 组。
...
全量 75 个测试文件使用本地 forks 池并行执行；CI 额外包含 PostgreSQL 集成组、覆盖率、Agent 3.0 评测快照、构建与依赖审计，工作流超时上限为 20 分钟。
...
| --- | --- | --- |
| 工程可回归 | ✅ 已建立 | 75 个测试文件、424 个用例；覆盖率防倒退；CSP enforce；生产依赖审计；关键 UI 回归 |
