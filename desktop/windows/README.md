...
用户已明确允许暂不签名、暂不启用自动更新。1.0.4 使用手动下载安装，网页下载弹窗明确提示未签名；不会要求关闭 Windows 安全防护。发布门禁通过后以 `windows-v1.0.4` 预发布（非全仓 Latest）分发普通和内嵌 WebView2 安装器两包。下方签名约束适用于未来正式稳定版，不代表此测试版已签名。
