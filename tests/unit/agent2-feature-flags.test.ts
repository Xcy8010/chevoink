...
describe('Agent 2.0 灰度与独立功能开关', () => {
  it('空灰度名单默认全量，单项开关互不影响', () => {
    ...
    expect(flags).toMatchObject({
      variant: 'v2', volume: true, changeSet: true, memory2: false, skill2: true, dualWorkspace: true,
    })
  })
  ...
  it('关闭开关时对应工具不会暴露给模型', () => {
    const tools = getToolsForAgent(getAgentDefinition('orchestrator'), 'build', {
      variant: 'v2', volume: false, changeSet: false, memory2: false, skill2: false, dualWorkspace: true,
    })
