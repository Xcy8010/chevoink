...
describe('Work workspace navigation and context', () => {
  ...
  it('includes the complete structure, plans, creative settings and session context', () => {
    ...
    expect(markup).toContain('作品结构 · 2 卷 2 章')
    expect(markup).toContain('第 1 卷 · 事故与逃亡')
    expect(markup).toContain('第 2 卷 · 门后的真相')
    expect(markup).toContain('第十三至十五章大纲')
    expect(markup).toContain('科幻悬疑')
    expect(markup).toContain('续写第二卷')
  })
})
