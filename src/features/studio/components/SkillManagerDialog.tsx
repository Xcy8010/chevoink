...
function SkillEditor({ editor, setEditor, submitting, submitLabel, importMode, onCancel, onSubmit }: {
  ...
  return <form onSubmit={submit} className="mx-auto max-w-2xl space-y-4">
    <div className="grid gap-3 md:grid-cols-2"><label className="text-xs text-[var(--text-secondary)]">名称<TextInput className="mt-1" value={editor.name} onChange={(event) => setEditor({ ...editor, name: event.target.value })} /></label><label className="text-xs text-[var(--text-secondary)]">语义版本<TextInput className="mt-1" value={editor.version} disabled={submitLabel === '创建草稿'} onChange={(event) => setEditor({ ...editor, version: event.target.value })} /></label></div>
    <label className="block text-xs text-[var(--text-secondary)]">用途<textarea rows={2} className={`${textareaClass} mt-1`} value={editor.description} onChange={(event) => setEditor({ ...editor, description: event.target.value })} /></label>
    <label className="block text-xs text-[var(--text-secondary)]">作用阶段<select className={`${inputClass} mt-1`} value={editor.phase} onChange={(event) => setEditor({ ...editor, phase: event.target.value as AgentSkillPhase })}>{phases.map((phase) => <option key={phase.value} value={phase.value}>{phase.label}</option>)}</select></label>
    {importMode ? <div className="grid gap-3 md:grid-cols-3">
      ...
      <label className="text-xs text-[var(--text-secondary)]">固定来源包<TextInput className="mt-1" placeholder="owner/repo@commit" value={editor.sourcePackage} onChange={(event) => setEditor({ ...editor, sourcePackage: event.target.value })} /></label>
      <label className="text-xs text-[var(--text-secondary)]">归属说明<TextInput className="mt-1" placeholder="原作者与修改说明" value={editor.attribution} onChange={(event) => setEditor({ ...editor, attribution: event.target.value })} /></label>
    </div> : null}
    <div className="grid gap-3 md:grid-cols-2"><label className="text-xs text-[var(--text-secondary)]">触发短语（每行一个）<textarea rows={5} className={`${textareaClass} mt-1`} value={editor.triggerPhrases} onChange={(event) => setEditor({ ...editor, triggerPhrases: event.target.value })} /></label><label className="text-xs text-[var(--text-secondary)]">明确不触发（每行一个）<textarea rows={5} className={`${textareaClass} mt-1`} value={editor.negativeTriggerPhrases} onChange={(event) => setEditor({ ...editor, negativeTriggerPhrases: event.target.value })} /></label></div>
    <label className="block text-xs text-[var(--text-secondary)]">技能规则与示例<textarea rows={10} className={`${textareaClass} mt-1`} value={editor.instructions} onChange={(event) => setEditor({ ...editor, instructions: event.target.value })} /></label>
