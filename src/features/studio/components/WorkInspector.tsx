...
function ContextTree({ novelTitle, volumeTitle, chapterTitle, chapterCount, wordCount, activeArtifactTitle, selectedTextLength = 0 }: Pick<Props, 'novelTitle' | 'volumeTitle' | 'chapterTitle' | 'chapterCount' | 'wordCount' | 'activeArtifactTitle' | 'selectedTextLength'>) {
  ...
  return <div className="h-full overflow-y-auto px-3 py-4">
    ...
    {expanded ? <div className="ml-4 border-l border-[var(--border-subtle)] pl-2">
      {volumeTitle ? <div className="flex items-center gap-2 px-2 py-2 text-xs text-[var(--text-secondary)]"><BookCopy className="h-3.5 w-3.5" /><span className="truncate">{volumeTitle}</span></div> : null}
      <details open className="group rounded-[8px]">
        <summary className="flex cursor-pointer list-none items-center gap-2 px-2 py-2 text-xs text-[var(--text-primary)] hover:bg-[var(--surface-muted)]"><FileText className="h-3.5 w-3.5 text-[var(--text-secondary)]" /><span className="min-w-0 flex-1 truncate">{chapterTitle}</span><span className="text-[10px] text-[var(--text-tertiary)]">{wordCount}</span></summary>
        <div className="ml-5 border-l border-[var(--border-subtle)] px-3 py-2 text-[11px] leading-5 text-[var(--text-secondary)]"><p>当前正文与章节元数据</p><p>对话选区：{selectedTextLength > 0 ? `${selectedTextLength} 字` : '未选择'}</p></div>
      </details>
      {activeArtifactTitle ? <details className="rounded-[8px]"><summary className="cursor-pointer list-none px-2 py-2 text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-muted)]">当前产物</summary><p className="px-4 pb-2 text-[11px] leading-5 text-[var(--text-secondary)]">{activeArtifactTitle}</p></details> : null}
    </div> : null}
  </div>
}
...
export default function WorkInspector({ tab, onTabChange, workTree, novelTitle, volumeTitle, chapterTitle, chapterCount, wordCount, pendingReviewCount, activeArtifactTitle, selectedTextLength, activities = [], memoryGraph, compactNavigation = false, showNavigation = true }: Props) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-hidden">
      ...
      {tab === 'context' ? <ContextTree novelTitle={novelTitle} volumeTitle={volumeTitle} chapterTitle={chapterTitle} chapterCount={chapterCount} wordCount={wordCount} activeArtifactTitle={activeArtifactTitle} selectedTextLength={selectedTextLength} /> : null}
