...
export default function WorkInspector({ tab, onTabChange, workTree, novelTitle, chapterTitle, chapterCount, wordCount, pendingReviewCount, activeArtifactTitle, onOpenMemory, onOpenMemoryReview }: Props) {
  ...
  return <div className="flex h-full min-h-0 flex-col">
    <div className="flex h-11 shrink-0 items-end border-b border-[var(--border-subtle)] pl-10 pr-1">
      {tabs.map(({ key, label, icon: Icon }) => <button key={key} type="button" onClick={() => { onTabChange(key); if (key === 'memory') onOpenMemory() }} className={cn('flex h-10 min-w-0 flex-1 items-center justify-center gap-1 border-b-2 px-1 text-[10px] transition', tab === key ? 'border-[var(--text-primary)] text-[var(--text-primary)]' : 'border-transparent text-[var(--text-tertiary)] hover:text-[var(--text-primary)]')}><Icon className="h-3.5 w-3.5" /><span className="truncate">{label}</span></button>)}
    </div>
