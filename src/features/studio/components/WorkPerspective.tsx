...
export default function WorkPerspective({ taskRail, compactTaskRail, conversation, activityDock, inspector, viewer, leftOpen, rightOpen, taskWidth, inspectorWidth, viewerWidth, onToggleLeft, onToggleRight, inspectorTab, onSelectInspectorTab, onBeginResize }: Props) {
  const resolvedInspectorWidth = viewer ? Math.max(260, Math.round(inspectorWidth / 2)) : inspectorWidth
  return <div className="flex h-full min-h-0 overflow-hidden bg-[var(--surface-default)]">
    ...
    <main className="min-w-0 flex-1 overflow-hidden bg-[var(--surface-default)]">{conversation}</main>
    {activityDock && !viewer ? <aside className="hidden h-full min-h-0 w-[300px] shrink-0 border-l border-[var(--border-subtle)] bg-[var(--app-bg)] 2xl:block">{activityDock}</aside> : null}
    {viewer ? <section className="studio-resizable-panel relative h-full min-h-0 shrink-0 border-l border-[var(--border-subtle)]" style={{ width: viewerWidth }}><PanelResizeHandle panel="workViewer" side="left" label="调整查看器宽度" onBegin={onBeginResize} />{viewer}</section> : null}
