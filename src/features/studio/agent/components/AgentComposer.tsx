          ...
          <details ref={modelMenuRef} className="group/model relative z-[120] ml-auto" data-disabled={running || disabled || undefined} onToggle={(event) => { if (!(event.currentTarget as HTMLDetailsElement).open) { setMobileModelsOpen(false); setEditingReasoningTier(null) } }}>
            ...
            <div className={cn('absolute bottom-full right-0 z-[140] mb-2 w-[248px] overflow-visible rounded-[14px] border border-[var(--border-subtle)] bg-[var(--surface-default)] p-1.5 shadow-[0_18px_46px_rgba(15,23,42,0.18)] motion-safe:origin-bottom-right motion-safe:animate-[agent-menu-in_150ms_cubic-bezier(0.2,0.8,0.2,1)]', mobileModelsOpen && 'max-sm:hidden')}>
              <div className="group/models relative">
                ...
                <div className={cn('invisible absolute bottom-0 right-[calc(100%-1px)] z-[150] w-60 translate-x-1 rounded-[14px] border border-[var(--border-subtle)] bg-[var(--surface-default)] p-1.5 opacity-0 shadow-[0_18px_46px_rgba(15,23,42,0.2)] transition-[opacity,transform,visibility] duration-150 ease-out group-hover/models:visible group-hover/models:translate-x-0 group-hover/models:opacity-100 group-focus-within/models:visible group-focus-within/models:translate-x-0 group-focus-within/models:opacity-100 max-sm:left-0 max-sm:right-0 max-sm:w-auto max-sm:translate-x-0 max-sm:overflow-y-auto max-sm:max-h-72 max-sm:transition-none', mobileModelsOpen ? 'max-sm:visible max-sm:opacity-100' : 'max-sm:hidden')}>
                  <button type="button" onClick={(event) => { event.currentTarget.blur(); setEditingReasoningTier(null); setMobileModelsOpen(false) }} className="mb-1 hidden h-8 w-full items-center gap-1.5 rounded-[8px] px-2 text-xs text-[var(--text-secondary)] hover:bg-[var(--surface-muted)] max-sm:flex" aria-label="返回模型菜单"><ChevronRight className="h-3.5 w-3.5 rotate-180" />返回</button>
                  ...
                  {modelOptions.filter((option) => option.available).map((option) => <div key={option.tier} className={cn('group/model-row flex h-9 w-full items-center rounded-[8px] hover:bg-[var(--surface-muted)]', option.tier === modelTier && 'bg-[var(--surface-muted)] font-medium')}><button type="button" onClick={() => { onModelTierChange(option.tier); modelMenuRef.current?.removeAttribute('open'); setEditingReasoningTier(null); setMobileModelsOpen(false) }} className="flex h-full min-w-0 flex-1 items-center gap-2 px-2.5 text-left text-xs"><span className="min-w-0 flex-1 truncate">{option.label}</span><span className="text-[10px] tabular-nums text-[var(--text-tertiary)] group-hover/model-row:hidden group-focus-within/model-row:hidden max-sm:hidden">{option.multiplier.toFixed(1)}x</span>{option.tier === modelTier ? <Check className="h-3.5 w-3.5 group-hover/model-row:hidden group-focus-within/model-row:hidden" /> : null}</button><button type="button" onClick={(event) => { event.stopPropagation(); setEditingReasoningTier(option.tier) }} className="mr-1 hidden h-7 items-center gap-1 rounded-[7px] px-2 text-[10px] text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-default)] group-hover/model-row:inline-flex group-focus-within/model-row:inline-flex max-sm:inline-flex" aria-label={`编辑${option.label}思考模式`}><Pencil className="h-3 w-3" />编辑</button></div>)}
