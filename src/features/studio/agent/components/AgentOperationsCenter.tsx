...
import type { AgentEvalComparisonView, AgentScheduleView, AgentSession, AgentSessionToolPolicy, AgentSandboxMode, AgentSubtaskRole, AgentSubtaskView, StoryBranchDiffView, StoryBranchView } from '../../../../../shared/contracts'
import {
  cancelAgentSubtaskRequest, createAgentScheduleRequest, createAgentSubtaskRequest, createEvalComparisonRequest,
  createStoryBranchRequest, fetchAgentSchedules, fetchAgentSessions, fetchAgentSubtasks, fetchEvalComparisons,
...
export default function AgentOperationsCenter({ open, onClose, novelId, sessionId, chapterId, runIds, currentSession, onSelectSession }: Props) {
  ...
  const [subtasks, setSubtasks] = useState<AgentSubtaskView[]>([])
  const [role, setRole] = useState<AgentSubtaskRole>('research')
  ...
  const refreshSubtasks = () => fetchAgentSubtasks(novelId).then((data) => setSubtasks(data.items))
  ...
  useEffect(() => {
    ...
    void execute(async () => { await Promise.all([refreshTasks(), refreshBranches(), refreshSubtasks(), refreshSchedules(), refreshEvals()]) })
      ...
      <section role="dialog" aria-modal="true" aria-label="Agent 操作中心" className="flex h-[92dvh] w-full max-w-5xl flex-col overflow-hidden border border-[var(--border-subtle)] bg-[var(--surface-default)] shadow-[0_28px_90px_rgba(15,23,42,0.28)] sm:h-[78dvh] sm:rounded-[24px]">
        ...
        <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
          ...
          <main className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            {tab === 'agents' ? <section><h3 className="text-sm font-semibold">专业子 Agent</h3><div className="mt-4 grid gap-3 sm:grid-cols-[180px_1fr]"><select className={inputClass} value={role} onChange={(event) => setRole(event.target.value as AgentSubtaskRole)}><option value="research">调研</option><option value="continuity">一致性</option><option value="quality">质量</option><option value="lore">设定</option></select><input className={inputClass} type="number" min={500} max={32000} step={500} value={tokenBudget} onChange={(event) => setTokenBudget(Number(event.target.value))} aria-label="Token 预算" /></div><textarea className={`${textareaClass} mt-3`} value={subtaskPrompt} onChange={(event) => setSubtaskPrompt(event.target.value)} placeholder="描述子 Agent 要完成的独立任务…" /><Button className="mt-3" disabled={!sessionId || !subtaskPrompt.trim() || busy} onClick={() => void execute(async () => { if (!sessionId) return; await createAgentSubtaskRequest({ novelId, parentSessionId: sessionId, chapterId, role, prompt: subtaskPrompt, tokenBudget }); setSubtaskPrompt(''); await refreshSubtasks() })}>启动子 Agent</Button><div className="mt-5 divide-y divide-[var(--border-subtle)] border-y border-[var(--border-subtle)]">{subtasks.map((item) => <div key={item.id} className="flex items-center gap-3 py-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{item.role} · {item.prompt}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">{item.status} · 预算 {item.tokenBudget.toLocaleString()} tokens</p></div>{item.traceUrl ? <a className="text-xs hover:underline" href={item.traceUrl} target="_blank" rel="noreferrer">轨迹</a> : null}{['running', 'queued', 'awaiting_approval'].includes(item.status) ? <Button size="sm" variant="ghost" onClick={() => void execute(async () => { await cancelAgentSubtaskRequest(item.id); await refreshSubtasks() })}>取消</Button> : null}</div>)}</div></section> : null}
