...
export function CustomModelSettingsContent({ active = true }: { active?: boolean }) {
  ...
  return <div><div className="mb-4 flex items-center justify-between"><div><p className="text-sm font-medium">自定义模型</p><p className="mt-1 text-xs text-[var(--text-secondary)]">API Key 使用加密存储，保存后只允许替换。</p></div><Button variant="secondary" size="sm" onClick={() => beginEdit('new')}><Plus className="h-4 w-4" />添加模型</Button></div>{query.isLoading ? <div className="flex justify-center py-12"><LoaderCircle className="h-5 w-5 animate-spin" /></div> : query.data?.models.length ? <div className="divide-y divide-[var(--border-subtle)] border-y border-[var(--border-subtle)]">{query.data.models.map((model) => <div key={model.id} className="flex items-center gap-3 py-4"><KeyRound className="h-4 w-4 shrink-0 text-[var(--text-secondary)]" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{model.displayName}</p><p className="mt-0.5 truncate text-xs text-[var(--text-tertiary)]">{model.modelName} · {model.defaultReasoningEffort}{model.visionEnabled ? ' · 视觉' : ''} · {model.enabled ? '已启用' : '已停用'}</p></div>{deletingId === model.id ? <div className="flex items-center gap-1"><Button size="sm" variant="ghost" onClick={() => setDeletingId(null)}>取消</Button><Button size="sm" variant="primary" className="bg-rose-700 hover:bg-rose-800" disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(model.id)}>确认删除</Button></div> : <><button type="button" onClick={() => beginEdit(model)} className="inline-flex h-8 w-8 items-center justify-center rounded-[8px] hover:bg-[var(--surface-muted)]" aria-label="编辑"><Pencil className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setDeletingId(model.id)} className="inline-flex h-8 w-8 items-center justify-center rounded-[8px] text-[var(--text-tertiary)] hover:bg-[var(--surface-muted)] hover:text-rose-600" aria-label="删除"><Trash2 className="h-3.5 w-3.5" /></button></>}</div>)}</div> : <div className="rounded-[14px] border border-dashed border-[var(--border-strong)] py-12 text-center text-sm text-[var(--text-tertiary)]">还没有自定义模型</div>}</div>
}
