...
export function CustomModelSettingsContent({ active = true }: { active?: boolean }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const baseUrl = form.provider === 'custom' ? form.baseUrl.trim() : selectedProvider.baseUrl
  ...
  if (editing) return <div className="pb-2">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-xs">显示名称<TextInput className="mt-1.5" value={form.displayName} onChange={(event) => setForm((value) => ({ ...value, displayName: event.target.value }))} placeholder="例如：我的高速模型" /></label>
      <label className="text-xs sm:col-span-2">模型 ID<TextInput className="mt-1.5" value={form.modelName} onChange={(event) => setForm((value) => ({ ...value, modelName: event.target.value }))} placeholder="由供应商提供的 model 名称" /></label>
      {form.provider === 'custom' ? <label className="text-xs sm:col-span-2">Base URL<TextInput className="mt-1.5" value={form.baseUrl} onChange={(event) => setForm((value) => ({ ...value, baseUrl: event.target.value }))} placeholder="https://api.example.com/v1" /></label> : <div className="sm:col-span-2 rounded-[12px] border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-3 py-2 text-xs text-[var(--text-secondary)]">已使用 {selectedProvider.label} 的官方兼容接口，无需填写 Base URL。</div>}
      <label className="text-xs sm:col-span-2">API Key<TextInput className="mt-1.5" type="password" value={form.apiKey} onChange={(event) => setForm((value) => ({ ...value, apiKey: event.target.value }))} placeholder={editing === 'new' ? '填写 API Key' : '已配置；留空保持不变'} /></label>
    </div>
