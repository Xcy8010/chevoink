    ...
    <AdminPanelState state={query.isLoading ? 'loading' : query.isError ? 'error' : 'ready'}>
      {data ? <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          {[['公测用户', data.summary.users], ['每日总额度', data.summary.dailyAllowance], ['今日已用', data.summary.dailyUsed], ['奖励余额', data.summary.bonusBalance], ['已耗尽用户', data.summary.exhaustedUsers]].map(([label, value]) => <AdminCard key={label}><p className="text-xs text-[var(--text-secondary)]">{label}</p><p className="mt-1 text-xl font-semibold tabular-nums">{Number(value).toLocaleString('zh-CN')}</p></AdminCard>)}
        </div>
        <AdminCard>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-sm font-semibold">全局状态</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">{data.summary.globallyPaused ? '当前已暂停，用户调用会被服务端拒绝。' : '当前正常开放，额度规则实时生效。'}</p></div><div className="flex gap-2"><Button onClick={() => setPending({ kind: 'reset-all' })}><RefreshCcw className="h-4 w-4" />重置全体</Button><Button variant={data.summary.globallyPaused ? 'primary' : 'secondary'} onClick={() => setPending({ kind: 'pause-all', paused: !data.summary.globallyPaused })}>{data.summary.globallyPaused ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />}{data.summary.globallyPaused ? '恢复调用' : '暂停全部'}</Button></div></div>
        </AdminCard>
        <AdminCard>
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[980px] text-left text-sm"><thead className="text-xs text-[var(--text-secondary)]"><tr><th className="pb-2"><input type="checkbox" checked={allSelected} onChange={(event) => setSelected(event.target.checked ? new Set(data.users.map((item) => item.user.id)) : new Set())} aria-label="选择全部用户" /></th><th>用户</th><th>套餐</th><th>状态</th><th>使用率</th><th>今日已用</th><th>奖励余额</th><th>当前可用</th><th /></tr></thead><tbody className="divide-y divide-[var(--border-default)]">{data.users.map((item) => <tr key={item.user.id}><td className="py-2.5"><input type="checkbox" checked={selected.has(item.user.id)} onChange={(event) => setSelected((value) => { const next = new Set(value); if (event.target.checked) next.add(item.user.id); else next.delete(item.user.id); return next })} aria-label={`选择 ${item.user.nickname}`} /></td><td><Link className="font-medium hover:underline" to={`/admin/users/${item.user.id}`}>{item.user.nickname}</Link></td><td><span className="rounded-full border border-[var(--border-subtle)] px-2 py-0.5 text-xs">{item.planLabel}</span></td><td><span className={item.suspended ? 'text-rose-600' : 'text-emerald-600'}>{item.suspended ? '已暂停' : '正常'}</span></td><td><Ring value={item.usedPercent} /></td><td className="tabular-nums">{item.dailyUsed.toLocaleString('zh-CN')} / {item.dailyAllowance.toLocaleString('zh-CN')}</td><td className="tabular-nums">{item.bonusBalance.toLocaleString('zh-CN')}</td><td className="tabular-nums">{item.totalRemaining.toLocaleString('zh-CN')}</td><td className="text-right"><div className="flex justify-end gap-1"><Button size="sm" onClick={() => setPending({ kind: 'reset-user', userId: item.user.id, name: item.user.nickname })}>重置</Button><Button size="sm" onClick={() => setPending({ kind: 'pause-user', userId: item.user.id, name: item.user.nickname, paused: !item.suspended })}>{item.suspended ? <Play className="h-3.5 w-3.5" /> : <Pause className="h-3.5 w-3.5" />}{item.suspended ? '恢复' : '暂停'}</Button></div></td></tr>)}</tbody></table></div>
        </AdminCard>
      </div> : null}
    </AdminPanelState>
