...
import { BrainCircuit, Eye, KeyRound, Pencil, X } from 'lucide-react'
  ...
  return <div>
    ...
    <AdminPanelState state={query.isLoading ? 'loading' : query.isError ? 'error' : 'ready'}>
      {query.data ? <div className="space-y-4">
        ...
        <div className="grid gap-3 lg:grid-cols-2">{query.data.models.map((model) => <AdminCard key={model.id}>
          <div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{model.displayName}</h2><span className="text-xs text-[var(--text-tertiary)]">{model.multiplier.toFixed(1)}x</span>{model.isDefault ? <span className="rounded-full border border-[var(--border-subtle)] px-2 py-0.5 text-[10px]">默认</span> : null}<span className={`rounded-full px-2 py-0.5 text-[10px] ${model.configurationReady ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{model.configurationReady ? '配置完整' : '待配置'}</span></div><p className="mt-1 text-xs text-[var(--text-secondary)]">{model.provider} · {model.modelName}</p></div><Button size="sm" onClick={() => setEditing(model)}><Pencil className="h-3.5 w-3.5" />编辑</Button></div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-[var(--text-secondary)]"><span>{model.enabled ? '服务已启用' : '服务已停用'}</span><span>{model.selectable ? '用户可选择' : '用户不可选'}</span><span className="inline-flex items-center gap-1"><BrainCircuit className="h-3.5 w-3.5" />{model.reasoningEfforts.join(' / ')}</span>{model.visionEnabled ? <span className="inline-flex items-center gap-1"><Eye className="h-3.5 w-3.5" />图片输入</span> : null}<span className="ml-auto inline-flex items-center gap-1"><KeyRound className="h-3.5 w-3.5" />{model.apiKeyConfigured ? '密钥已配置' : model.tier === 'speed' ? '沿用环境密钥' : '未配置密钥'}</span></div>
        </AdminCard>)}</div>
      </div> : null}
    </AdminPanelState>
